...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('B1' in ix_):
            pb.x0[ix_['B1']] = float(-2000.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['B1']),float(-2000.0)))
        if('B2' in ix_):
            pb.x0[ix_['B2']] = float(50.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['B2']),float(50.0)))
        if('B3' in ix_):
            pb.x0[ix_['B3']] = float(0.8)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['B3']),float(0.8)))
        pass
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        [it,iet_,_] = s2x_ii( 'eE15', iet_)
        ...
        for I in range(int(v_['1']),int(v_['M'])+1):
            ...
            pbm.elftype = arrset(pbm.elftype,ie,'eE15')
    ...
    @staticmethod
    def eE15(pbm,nargout,*args):

        import numpy as np
        EV_  = args[0]
        iel_ = args[1]
        V3INV = 1.0/EV_[2]
        V2PX = EV_[1]+pbm.elpar[iel_][0]
        V2PXL = np.log(V2PX)
        V2PXP = V2PX**V3INV
