...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('X1' in ix_):
            ...
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X1']),float(1.3)))
        if('X2' in ix_):
            pb.x0[ix_['X2']] = float(0.65)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X2']),float(0.65)))
        if('X3' in ix_):
            pb.x0[ix_['X3']] = float(0.65)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X3']),float(0.65)))
        if('X4' in ix_):
            pb.x0[ix_['X4']] = float(0.7)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X4']),float(0.7)))
        if('X5' in ix_):
            pb.x0[ix_['X5']] = float(0.6)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X5']),float(0.6)))
        if('X6' in ix_):
            pb.x0[ix_['X6']] = float(3.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X6']),float(3.0)))
        if('X7' in ix_):
            pb.x0[ix_['X7']] = float(5.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X7']),float(5.0)))
        if('X8' in ix_):
            pb.x0[ix_['X8']] = float(7.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X8']),float(7.0)))
        if('X9' in ix_):
            pb.x0[ix_['X9']] = float(2.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X9']),float(2.0)))
        if('X10' in ix_):
            pb.x0[ix_['X10']] = float(4.5)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X10']),float(4.5)))
        if('X11' in ix_):
            pb.x0[ix_['X11']] = float(5.5)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X11']),float(5.5)))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
