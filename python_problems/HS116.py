...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        pbm.A[ig,iv] = float(1.0)+pbm.A[ig,iv]
        iv = ix_['X2']
        pbm.A[ig,iv] = float(-0.03475)+pbm.A[ig,iv]
        ...
        pbm.gconst = arrset(pbm.gconst,ig_['C4'],float(50.0))
        pbm.gconst = arrset(pbm.gconst,ig_['C9'],float(-1.0))
        pbm.gconst = arrset(pbm.gconst,ig_['C11'],float(0.9))
        #%%%%%%%%%%%%%%%%%%%%  RANGES %%%%%%%%%%%%%%%%%%%%%%
        ...
        if('X2' in ix_):
            ...
            pb.y0 = arrset(pb.y0,find(pbm.congrps,lambda x:x==ig_['X2']),float(0.8))
        if('X3' in ix_):
            pb.x0[ix_['X3']] = float(0.9)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X3']),float(0.9)))
        if('X4' in ix_):
            pb.x0[ix_['X4']] = float(0.1)
        else:
            pb.y0 = arrset(pb.y0,find(pbm.congrps,lambda x:x==ig_['X4']),float(0.1))
        if('X5' in ix_):
            pb.x0[ix_['X5']] = float(0.14)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X5']),float(0.14)))
        if('X6' in ix_):
            pb.x0[ix_['X6']] = float(0.5)
        else:
            pb.y0 = arrset(pb.y0,find(pbm.congrps,lambda x:x==ig_['X6']),float(0.5))
        if('X7' in ix_):
            pb.x0[ix_['X7']] = float(489.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X7']),float(489.0)))
        if('X8' in ix_):
            pb.x0[ix_['X8']] = float(80.0)
        else:
            pb.y0 = arrset(pb.y0,find(pbm.congrps,lambda x:x==ig_['X8']),float(80.0))
        if('X9' in ix_):
            pb.x0[ix_['X9']] = float(650.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X9']),float(650.0)))
        if('X10' in ix_):
            pb.x0[ix_['X10']] = float(450.0)
        else:
            pb.y0 = arrset(pb.y0,find(pbm.congrps,lambda x:x==ig_['X10']),float(450.0))
        if('X11' in ix_):
            pb.x0[ix_['X11']] = float(150.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X11']),float(150.0)))
        if('X12' in ix_):
            pb.x0[ix_['X12']] = float(150.0)
        else:
            pb.y0 = arrset(pb.y0,find(pbm.congrps,lambda x:x==ig_['X12']),float(150.0))
        if('X13' in ix_):
            pb.x0[ix_['X13']] = float(150.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['X13']),float(150.0)))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
        ...
        pbm.grelw = loaset(pbm.grelw,ig,posel,float(1.231059))
        ig = ig_['C6']
        posel = len(pbm.grelt[ig])
        pbm.grelt = loaset(pbm.grelt,ig,posel,ie_['E2'])
        nlc = np.union1d(nlc,np.array([ig]))
        pbm.grelw = loaset(pbm.grelw,ig,posel,float(-0.975))
        posel = posel+1
