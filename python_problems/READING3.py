...
# ********************
    ...
    def eENERGY(self, nargout,*args):
        ...
        f_   = self.elpar[iel_][1]*EV_[0,0]*(EV_[1,0]-C2PIT)**2
        if nargout>1:
            try:
                dim = len(IV_)
            except:
                dim = len(EV_)
            g_ = np.zeros(dim)
            g_[0] = self.elpar[iel_][1]*(EV_[1,0]-C2PIT)**2
            g_[1] = self.elpar[iel_][1]*2.0*EV_[0,0]*(EV_[1,0]-C2PIT)
            if nargout>2:
                H_ = np.zeros((2,2))
                H_[1,0] = self.elpar[iel_][1]*2.0*(EV_[1,0]-C2PIT)
                H_[0,1] = H_[1,0]
                H_[1,1] = self.elpar[iel_][1]*2.0*EV_[0,0]
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
