...
# **********************
    ...
    def eA(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        f_   = self.elpar[iel_][0]*(EV_[1,0]-2.0*EV_[0,0]**2+1.0)
        if nargout>1:
            try:
                dim = len(IV_)
            except:
            ...
            g_[0] = -4.0*self.elpar[iel_][0]*EV_[0,0]
            if nargout>2:
                H_ = np.zeros((2,2))
    ...
    def eB(self, nargout,*args):
        ...
        EV_  = args[0]
        iel_ = args[1]
        f_   = self.elpar[iel_][0]*(EV_[1,0]-2.0*EV_[0,0]**2+1.0)*EV_[0,0]
        if nargout>1:
            try:
                dim = len(IV_)
            except:
            ...
            g_[0] = self.elpar[iel_][0]*(EV_[1,0]-6.0*EV_[0,0]**2+1.0)
            if nargout>2:
                H_ = np.zeros((2,2))
                H_[1,0] = self.elpar[iel_][0]
                H_[0,1] = H_[1,0]
                H_[0,0] = -12.0*self.elpar[iel_][0]*EV_[0,0]
        if nargout == 1:
            return f_
        elif nargout == 2:
            return f_,g_
