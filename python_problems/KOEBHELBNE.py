...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    def __init__(self, *args): 
        ...
        if('N' in ix_):
            pb.x0[ix_['N']] = float(10.0)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['N']),float(10.0)))
        if('A' in ix_):
            pb.x0[ix_['A']] = float(0.01)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['A']),float(0.01)))
        if('B' in ix_):
            pb.x0[ix_['B']] = float(0.01)
        else:
            pb.y0  = (
                  arrset(pb.y0,findfirst(pbm.congrps,lambda x:x==ig_['B']),float(0.01)))
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        iet_  = {}
        elftv = []
    ...
    @staticmethod
    def eKHE(pbm,nargout,*args):

        import numpy as np
        EV_  = args[0]
        iel_ = args[1]
        T = np.exp(-EV_[1]/pbm.elpar[iel_][0]-pbm.elpar[iel_][0]/EV_[2])
        M1OX = -1.0/pbm.elpar[iel_][0]
        XOB2 = pbm.elpar[iel_][0]/(EV_[2]*EV_[2])
        M2XOB3 = -2.0*pbm.elpar[iel_][0]/EV_[2]**3
        f_   = EV_[0]*T
