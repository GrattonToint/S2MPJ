...
% IE N                   5000           $-PARAMETER  n=14999,m=9998
        ...
        pb.xnames = {};
        for T=v_('1'):v_('N-1')
            [iv,ix_] = s2xlib('ii',['X',int2str(T)],ix_);
            pb.xnames{iv} = ['X',int2str(T)];
        end
        for T=v_('1'):v_('N')
            [iv,ix_] = s2xlib('ii',['Y',int2str(T),',',int2str(round(v_('1')))],ix_);
            pb.xnames{iv} = ['Y',int2str(T),',',int2str(round(v_('1')))];
            [iv,ix_] = s2xlib('ii',['Y',int2str(T),',',int2str(round(v_('2')))],ix_);
        ...
        for T=v_('1'):v_('N-1')
            ...
            [ig,ig_] = s2xlib('ii',['TT',int2str(T),',',int2str(round(v_('2')))],ig_);
            gtype{ig}  = '==';
