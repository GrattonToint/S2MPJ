...
% IE N                   10000          $-PARAMETER
        ...
        for I=v_('1'):v_('4'):v_('N')
            ...
            [ig,ig_] = s2xlib('ii',['G',int2str(round(v_('I+1')))],ig_);
            gtype{ig} = '<>';
            ...
            gtype{ig} = '<>';
            pbm.gscale(ig,1) = 0.2;
            [ig,ig_] = s2xlib('ii',['G',int2str(round(v_('I+2')))],ig_);
            gtype{ig} = '<>';
            iv = ix_(['X',int2str(round(v_('I+1')))]);
            if(size(pbm.A,1)>=ig&&size(pbm.A,2)>=iv)
                pbm.A(ig,iv) = 1.0+pbm.A(ig,iv);
            ...
            [ig,ig_] = s2xlib('ii',['G',int2str(round(v_('I+3')))],ig_);
            gtype{ig} = '<>';
            iv = ix_(['X',int2str(I)]);
            if(size(pbm.A,1)>=ig&&size(pbm.A,2)>=iv)
                pbm.A(ig,iv) = 1.0+pbm.A(ig,iv);
