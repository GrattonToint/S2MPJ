...
switch(action)
    ...
    pbm.name     = 'ODFITS';
        ...
        %%%%%%%%%%%%%%%%%%%%  VARIABLES %%%%%%%%%%%%%%%%%%%%
        pb.xnames = {};
        [iv,ix_] = s2xlib('ii','T13',ix_);
        pb.xnames{iv} = 'T13';
        [iv,ix_] = s2xlib('ii','T14',ix_);
        pb.xnames{iv} = 'T14';
        [iv,ix_] = s2xlib('ii','T23',ix_);
        pb.xnames{iv} = 'T23';
        [iv,ix_] = s2xlib('ii','T24',ix_);
        pb.xnames{iv} = 'T24';
        for I=v_('1'):v_('ARCS')
        ...
        for I=v_('1'):v_('ARCS')
            [ig,ig_] = s2xlib('ii',['CP',int2str(I)],ig_);
            gtype{ig} = '<>';
            iv = ix_(['F',int2str(I)]);
            if(size(pbm.A,1)>=ig&&size(pbm.A,2)>=iv)
                pbm.A(ig,iv) = -1.0+pbm.A(ig,iv);
