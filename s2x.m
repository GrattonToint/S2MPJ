...
function [ probname, exitc, errors ] = s2x( sifpbname, varargin )

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%   S2X: a SIF decoder to Matlab, Python and Julia
%
...
%   of the constraints has its rows defined as the gradients of individual constraints. When S2X produces a Matlab file,
...
%   - adds a final call to the S2X-supplied s2xlib.m (for Matlab), s2xlib.py (for Python) or s2xlib.jl (for Julia) that
%     accesses the previously defined actions and data-structures to perform the required evaluation tasks.
%   The decoder then produces an output file called 'probname.m', 'probname.py' or 'probname.jl' in the current directory,
%   possibly overwriting an existing file with the same name (see the S2X OPTIONS section below to modify the directory
...
%   variables, a vector of constraints), S2X transforms variables, groups and elements into linear 'flat' unidimensional
%   structures. Since this transformation may depend on problem internal parameters such as loop limits, themselves
%   depending on problem input arguments only known at runtime, the output file uses the S2X-supplied function s2x_ii(...)
%   to compute the relevant indices within 'setup'/__init__ at runtime.
%
%   The names of SIF entities have a potentially complex structure, possibly involving symbolic or numerical indices, but
%   also numerical digits within the names which may (or may not) correspond to indeces. S2X attempts to sort this out in
%   the s2xname function, in order to assign a Matlab/Python/Julia-comptatible name to each of these entities. These names
%   are then used in a dictionaries associating names and values (v_ for parameters) or index (ix_ for variables, ig_ for
%   groups, iet_ for element types, ie_ for elements and itg_ for group types).
%
%   Note that the SIF file may also include scaling factors for the variables of the groups' linear terms.  These are read
%   by S2X and are, by default, applied to the relevant coefficients during 'setup', in order to speed up later evaluation
%   calls. Again this *differs* from what is done by the historical Fortran decoder, in which variable scaling is ignored
%   entirely (this less efficient approach may nevertheless be requested as an option).
%
...
%   data structure just before the main call to evaluations, and then checked for within s2xlib (meaning that the 'e@glob'
%   and 'g@globs' actions may be called there if needed).
%
...
%        Matlab s2x to pass information on entities like loops, element/ group ftype(s) across the different
...
%        successive calls) and the nonlinear functions' evaluation functions/methods in s2xlib.
%
%   Input:
%
%      sifpbname  : a string containing the name of the problem to be decoded (S2X then reads the 'probname.SIF' file for
%                   input).
%      varargin{1}: if present, vararagin{1} allows the specification of S2X options (see the section on S2X OPTIONS
...
%           
%      Beyond those arguments, S2X typically produces an output file called 'probname.m', 'probname.py'  or 'probname.jl'
...
%                      decoder without need for further user action (see the pbxscale option of s2x below)
...
%
%         This is only for convenience, but this 'ordered' feature  should not be included in a 'S2X format' for problem
%         description, the type of each constraint being determined by the values the lower and upper bounds on its value.
%         This may also be superseded by setting the keepcorder flag in the S2X options (see below). If the problem has no
...
%         and objupper may also be missing if no value is provided in the SIF file. When specific S2X options are used
...
%         fields enames and gnames in the pbm struct depends on the specific S2X options defined (see below).
%
...
%     CUTEst_problem class described in the s2xlib.py file. Its inherits the evaluation methods of this parent class,
...
%
%   The use of S2X is typically as follows.
%   1) a SIF problem (PROBLEM.SIF, say) is decoded by the Matlab command
%
%         PROBLEM = s2x( 'SIFPROBLEM' );
%
%      or
%
%         PROBLEM = s2x( 'SIFPROBLEM', options );
%
%      where options is a struct whose content is described below in the S2X OPTIONS paragraph. This assumes that the
...
%
%       PROBNAME = s2x( 'SIFPBNAME', inpy )
%
%   where inpy is a struct describing the S2X options (see next paragraph) with inpy.language = 'python' (other options
...
%
%       PROBNAME = s2x( 'PROBNAME', injl )
%
%   where injl is a struct describing the S2X options (see next paragraph) with injl.language = 'julia' (other options
...
%   THE S2X OPTIONS
%
%   S2X gives the user some access to internal options, typically to control file production during execution, save memory,
...
%
%        * varargin{1}.language is is a string specifying the type of output produced by S2X:
...
%                     problems decoded with it, this option should be activated (preventing internal scaling within s2x)
...
%
%   In addition, S2X *DOES NOT* provide a comprehensive explanation of possible errors in the SIF file (it is supposed to
%   adhere to the standard): it may thus crash on errors, or the produced output file may also crash. Should this happen,
%   running S2X with varargin{1].showsiflines = 1, usually helps spotting the SIF error (or detecting the S2X bug).
%
...
%      getenames and getgnames to 1 (see S2X internal options above).
...
%   4) S2X makes the assumption that RANGES are only meaningful for inequality constraints. The SIF standard appears to
%      leave open the  possibility of ranges for other types of groups, but fails to mention what they can/could be used
%      for. Thus S2X ignores the ranges of equality constraints or objective groups, should them be supplied in the SIF
%      file.
%   5) S2X handles the constraints in the SIF format (in order to have coherent definition of constants for both the
%      objective and constraints, and only converts them to the S2X format (if requested) at the end of the 'setup'  or
...
%      used in S2X to avoid stating the SIF default for as long as one is certain that no other default is stated to
...
%  If S2X options are specified by the user, use them instead of the defaults.
...
if ( nargin > 1 )
   ...
   if ( isstruct( varargin{1} ) )
      ...
      pbs.errors{end+1} = 'ERROR: second argument of S2X is not a struct (options)';
      disp( pbs.errors{end} );
...
has_H           = 0; % true if a quadratic term idefined (in the QUADRATICS section)
   ...
   elseif ( line(1) == '*'  )
      ...
      if ( lline > 1 )
         ...
         if ( ~isempty( posc ) )
             switch ( pbs.lang )
             case 'matlab'
                classification = [ '''',   strtrim( line( posc+15:end ) ), '''' ];
...
%      probname = strtrim( line(15:end) );
      ...
      case 'matlab'
         ...
         printcmline( ' ',                                                                            bindent, pbs.fidma );
      case 'python'
         printpline( 'from s2xlib import *',                                                       0, bindent, pbs.fidpy );
         printpline( sprintf( 'class  %s(CUTEst_problem):', probname ),                            0, bindent, pbs.fidpy );
         printpline( ' ',                                                                          0, bindent, pbs.fidpy );
         printcpline( '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%',   bindent, pbs.fidpy );
         printcpline( ' ', bindent, pbs.fidpy );
      case 'julia'
         printjline( sprintf( 'function %s(action,args...)', probname ),                           0, bindent, pbs.fidjl );
         printcjline( ' ',                                                                            bindent, pbs.fidjl );
         printcjline( '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%',    bindent, pbs.fidjl );
         printcjline( ' ',                                                                            bindent, pbs.fidjl );
...
%         end
   ...
   elseif ( lline >= 10 && strcmp( line(1:10), 'GROUP USES' ) ) 
      ...
      case 'matlab'
         ...
         printmline( '[pbm.grelt{1:ngrp}] = deal(repmat([],1,ngrp));',                        indlvl, bindent, pbs.fidma );
   ...
   elseif( lline >= 6 && strcmp( line(1:6), 'ENDATA' ) )
      ...
      if ( indata )
         ...
         if ( has_constraints )
            ...
            %  2) In the better(?) S2X format: computate the lower and upper bounds on constraints values. 
   ...
   elseif ( indata )
      ...
      case 'DO'
         ...
         index         = s2xname( '', f{2}, pbs );      %  loop indeces
         iename        = s2xname( '', f{5}, pbs );
         ...
         case 'matlab'
            pending{1} = sprintf( 'for %s=%s:%s', ...
                         index(2:end-1), s2xvalue('', f{3}, pbs ), s2xvalue( '', f{5}, pbs ) );
         case 'python'
      ...
      case 'DI' 
         switch( pbs.lang )
         case 'matlab'
            printmline( replace( pending{1}, ':', sprintf( ':%s:', s2xvalue( '', f{3}, pbs ) ) ), ...
                                                                                              indlvl, bindent, pbs.fidma );
         case 'python'
            er = strfind( pending{1}, '):' );
            printpline( sprintf( '%s,int(%s)):', pending{1}(1:er-1), s2xvalue( '', f{3}, pbs ) ), ...
                                                                                              indlvl, bindent, pbs.fidpy );
         case 'julia'
            printjline( replace( pending{1}, ':', sprintf( ':Int64(%s):', s2xvalue( '', f{3}, pbs ) ) ), ...
                                                                                              indlvl, bindent, pbs.fidjl );
         end
         pending        = {};
         pendingkey     = '';
      ...
      %  screening the name of the relevant parameters using s2xname.
      ...
      case { 'RI', 'AI' }
         ...
         [ name2, isidx ]  = s2xname ( f{1}(1), f{2}, pbs );
         if ( isidx )
            switch ( pbs.lang )
            case 'matlab'
               name2 = [ '''', name2, '''' ];
      ...
      case 'VARIABLES'
         ...
         vname = s2xname( '', f{2}, pbs );
         printmline( sprintf( '[iv,ix_] = s2xlib(''ii'',%s,ix_);', vname ),                   indlvl, bindent, pbs.fidma );
         printpline( sprintf( '[iv,ix_,_] = s2x_ii(%s,ix_)', vname ),                         indlvl, bindent, pbs.fidpy );
         printjline( sprintf( 'iv,ix_,_ = s2x_ii(%s,ix_)', vname ),                           indlvl, bindent, pbs.fidjl );
         if ( getxnames )
      ...
      case 'DGROUPS'
         ...
         if ( isempty( f{1} ) )
            ...
            [ ~, act ] = formact( f{1} );
            [ gname, ~, explindex ]  = s2xname( '', f{2}, pbs);
            ...
            if ( ~strcmp( f{2}, prevgname ) || explindex )
               printmline( sprintf( '[ig,ig_] = s2xlib(''ii'',%s,ig_);', gname ),             indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[ig,ig_,_] = s2x_ii(%s,ig_)'      , gname ),             indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'ig,ig_,_ = s2x_ii(%s,ig_)'        , gname ),             indlvl, bindent, pbs.fidjl );
               if ( getgnames )
      ...
      case 'CONSTANTS'
         ...
         if ( strcmp( f{3}, '''DEFAULT''' ) )
            ...
            printpline( sprintf( 'pbm.gconst = arrset(pbm.gconst,ig_[%s],float(%s))', gname, getv1(f{1},f,pbs) ), ...
                                                                                              indlvl, bindent, pbs.fidpy );
            printjline( sprintf( 'pbm.gconst[ig_[%s]] = Float64(%s)', gname, getv1(f{1},f,pbs) ),      indlvl, bindent, pbs.fidjl );
            if ( nf > 5 )
               gname = s2xname( '', f{5}, pbs );
               printmline( sprintf( 'pbm.gconst(ig_(%s)) = %s;',                  gname, getv2(f) ), ...
                                                                                              indlvl, bindent, pbs.fidma );
               printpline( sprintf( 'pbm.gconst = arrset(pbm.gconst,ig_[%s],float(%s))', gname, getv2(f) ), ...
                                                                                              indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'pbm.gconst[ig_[%s]] = Float64(%s)',                   gname, getv2(f) ), ...
                                                                                              indlvl, bindent, pbs.fidjl ); 
            end
         end
         has_constants = 1;
...
%               has_xupper = 1;
      ...
      %  Element types are defined both in the S2X execution and in the output file.  The first is used in the analysis of
      %  the ELEMENTS (INDIVIDUALS) section while the second is need to assign ftype to elements. Note that the latter
      ...
      case 'ELFTYPE'
         ...
         case 'EV'
            if ( ~strcmp( prevetype, f{2} ) )
               printmline( sprintf( '[it,iet_] = s2xlib( ''ii'', ''%s'',iet_);', eftname ), ...
                                                                                              indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[it,iet_,_] = s2x_ii( ''%s'', iet_)', eftname ),         indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'it,iet_,_ = s2x_ii( "%s", iet_)' ,    eftname ),         indlvl, bindent, pbs.fidjl );
               prevetype = f{2};
            end                           
         ...
         case 'EP'
            ...
            if ( ~strcmp( prevetype, f{2} ) )
               printmline( sprintf( '[it,iet_] = s2xlib(''ii'',''%s'',iet_);', eftname ),     indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[it,iet_,_] = s2x_ii(''%s'',iet_)', eftname ),           indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'it,iet_,_ = s2x_ii("%s",iet_)', eftname ),               indlvl, bindent, pbs.fidjl ); 
               prevetype = f{2};
            end
      ...
      case 'ELUSES'
         ...
         %  s2xname and the flag explindex = 1 is then returned.

         [ ename,~,explindex] = s2xname( '', f{2}, pbs);
         if ( ~strcmp( f{2}, prevename ) || explindex ) 
            ...
            if ( ~isempty( defelftype ) )
               printmline( '[ie,ie_,newelt] = s2xlib(''ii'',ename,ie_);',                     indlvl, bindent, pbs.fidma );
               printpline( '[ie,ie_,newelt] = s2x_ii(ename,ie_)',                             indlvl, bindent, pbs.fidpy );
               printjline( 'ie,ie_,newelt = s2x_ii(ename,ie_)',                               indlvl, bindent, pbs.fidjl );
            else
               printmline( '[ie,ie_] = s2xlib(''ii'',ename,ie_);',                            indlvl, bindent, pbs.fidma );
               printpline( '[ie,ie_,_] = s2x_ii(ename,ie_)',                                  indlvl, bindent, pbs.fidpy );
               printjline( 'ie,ie_,_  = s2x_ii(ename,ie_)',                                   indlvl, bindent, pbs.fidjl );
            end
            if ( getenames )        
         ...
         case 'V'
            ...
            %  This task is performed by s2x_nlx,( ...) and the different calls are needed to reflect the fact that these
            ...
            if     (  isempty( xlowdef ) &&  isempty( xuppdef ) &&  isempty( x0def ) )
               printmline( sprintf( '[iv,ix_,pb] = s2xlib(''nlx'',vname,ix_,pb,%d,[],[],[]);',      ...
                           getxnames ),                                                       indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[iv,ix_,pb] = s2x_nlx(vname,ix_,pb,%d,None,None,None)',        ...
                           getxnames ),                                                       indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'iv,ix_,pb = s2x_nlx(vname,ix_,pb,%d,nothing,nothing,nothing)', ...
                           getxnames ),                                                       indlvl, bindent, pbs.fidjl );
            elseif ( ~isempty( xlowdef ) &&  isempty( xuppdef ) &&  isempty( x0def ) )
               printmline( sprintf( '[iv,ix_,pb] = s2xlib(''nlx'',vname,ix_,pb,%d,%s,[],[]);',      ...
                           getxnames, xlowdef ),                                              indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[iv,ix_,pb] = s2x_nlx(vname,ix_,pb,%d,%s,None,None)',          ...
                           getxnames, xlowdef ),                                              indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'iv,ix_,pb = s2x_nlx(vname,ix_,pb,%d,%s,nothing,nothing)',      ...
                           getxnames, xlowdef ),                                              indlvl, bindent, pbs.fidjl );
            elseif (  isempty( xlowdef ) && ~isempty( xuppdef ) &&  isempty( x0def ) )
               printmline( sprintf( '[iv,ix_,pb] = s2xlib(''nlx'',vname,ix_,pb,%d,[],%s,[]);',      ...
                           getxnames, xuppdef ),                                              indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[iv,ix_,pb] = s2x_nlx(vname,ix_,pb,%d,None,%s,None)',          ...
                           getxnames, xuppdef ),                                              indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'iv,ix_,pb = s2x_nlx(vname,ix_,pb,%d,nothing,%s,nothing)',      ...
                           getxnames, xuppdef ),                                              indlvl, bindent, pbs.fidjl );
            elseif (  isempty( xlowdef ) &&  isempty( xuppdef ) && ~isempty( x0def ) )
               printmline( sprintf( '[iv,ix_,pb] = s2xlib(''nlx'',vname,ix_,pb,%d,[],[],%s);',      ...
                           getxnames, x0def ),                                                indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[iv,ix_,pb] = s2x_nlx(vname,ix_,pb,%d,None,None,%s)',          ...
                           getxnames, x0def ),                                                indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'iv,ix_,pb = s2x_nlx(vname,ix_,pb,%d,nothing,nothing,%s)',      ...
                           getxnames, x0def ),                                                indlvl, bindent, pbs.fidjl );
            elseif ( ~isempty( xlowdef ) && ~isempty( xuppdef ) &&  isempty( x0def ) )
               printmline( sprintf( '[iv,ix_,pb] = s2xlib(''nlx'',vname,ix_,pb,%d,%s,%s,[]);',      ...
                           getxnames, xlowdef, xuppdef ),                                     indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[iv,ix_,pb] = s2x_nlx(vname,ix_,pb,%d,%s,%s,None)',            ...
                           getxnames, xlowdef, xuppdef ),                                     indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'iv,ix_,pb = s2x_nlx(vname,ix_,pb,%d,%s,%s,nothing)',           ...
                           getxnames, xlowdef, xuppdef ),                                     indlvl, bindent, pbs.fidjl );
            elseif ( ~isempty( xlowdef ) &&  isempty( xuppdef ) && ~isempty( x0def ) )
               printmline( sprintf( '[iv,ix_,pb] = s2xlib(''nlx'',vname,ix_,pb,%d,%s,[],%s);',      ...
                           getxnames, xlowdef, x0def ),                                       indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[iv,ix_,pb] = s2x_nlx(vname,ix_,pb,%d,%s,None,%s)',            ...
                           getxnames, xlowdef, x0def ),                                       indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'iv,ix_,pb = s2x_nlx(vname,ix_,pb,%d,%s,nothing,%s)',           ...
                           getxnames, xlowdef, x0def ),                                       indlvl, bindent, pbs.fidjl );
            elseif (  isempty( xlowdef ) && ~isempty( xuppdef ) && ~isempty( x0def ) )
               printmline( sprintf( '[iv,ix_,pb] = s2xlib(''nlx'',vname,ix_,pb,%d,[],%s,%s);',      ...
                           getxnames, xuppdef, x0def ),                                       indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[iv,ix_,pb] = s2x_nlx(vname,ix_,pb,%d,None,%s,%s)',            ... 
                           getxnames, xuppdef, x0def ),                                       indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'iv,ix_,pb = s2x_nlx(vname,ix_,pb,%d,nothing,%s,%s)',           ... 
                           getxnames, xuppdef, x0def ),                                       indlvl, bindent, pbs.fidjl );
            elseif ( ~isempty( xlowdef ) && ~isempty( xuppdef ) && ~isempty( x0def ) )
               printmline( sprintf( '[iv,ix_,pb] = s2xlib(''nlx'',vname,ix_,pb,%d,%s,%s,%s);', ...
                           getxnames, xlowdef, xuppdef, x0def ),                              indlvl, bindent, pbs.fidma );
               printpline( sprintf( '[iv,ix_,pb] = s2x_nlx(vname,ix_,pb,%d,%s,%s,%s)',         ...
                           getxnames, xlowdef, xuppdef, x0def ),                              indlvl, bindent, pbs.fidpy );
               printjline( sprintf( 'iv,ix_,pb = s2x_nlx(vname,ix_,pb,%d,%s,%s,%s)',           ...
      ...
      case 'GRFTYPE'
         ...
         printmline( sprintf( '[it,igt_] = s2xlib(''ii'',''%s'',igt_);', grftname ),          indlvl, bindent, pbs.fidma );
         printpline( sprintf( '[it,igt_,_] = s2x_ii(''%s'',igt_)',       grftname ),          indlvl, bindent, pbs.fidpy );
         printjline( sprintf( 'it,igt_,_ = s2x_ii("%s",igt_)',           grftname ),          indlvl, bindent, pbs.fidjl );
      ...
      case 'GRUSES'
         ...
         otherwise
            ...
            if ( nf > 2 )
               gname = s2xname('',f{2},pbs);    %  Note that the group must have been defined in the data GROUPS section.
      ...
      case 'OBOUND'
         ...
         case 'ZL'
            printmline( sprintf( 'pb.objlower = v_(%s);', s2xname('',f{5},pbs) ),             indlvl, bindent, pbs.fidma );
            printpline( sprintf( 'pb.objlower = v_[%s]',  s2xname('',f{5},pbs) ),             indlvl, bindent, pbs.fidpy );
            printjline( sprintf( 'pb.objlower = v_[%s]',  s2xname('',f{5},pbs) ),             indlvl, bindent, pbs.fidjl );
         case { 'UP', 'XU' }
         ...
         case 'ZU'
            printmline( sprintf( 'pb.objupper = v_(%s);', s2xname('',f{5},pbs) ),             indlvl, bindent, pbs.fidma );
            printpline( sprintf( 'pb.objupper = v_[%s]',  s2xname('',f{5},pbs) ),             indlvl, bindent, pbs.fidpy );
            printjline( sprintf( 'pb.objupper = v_[%s]',  s2xname('',f{5},pbs) ),             indlvl, bindent, pbs.fidjl );
         end
   ...
   elseif( ingroups )
      ...
      case 'GINDIVS'
         ...
         case 'A'
            ...
            switch( pbs.lang )
            case 'matlab'
               pending{1}   = sprintf( '%s = %s;', s2xname( 'u', f{2}, pbs ), from_fortran( f{4}, locdict, pbs ) );
            case { 'python', 'julia' }
               pending{1}   = sprintf( '%s = %s',  s2xname( 'u', f{2}, pbs ), from_fortran( f{4}, locdict, pbs ) );
            end
            pendingkey      = 'fortran';
            locdict( f{2} ) = s2xname( 'u', f{2}, pbs );
...
case 'matlab'
   ...
   printmline(                                                                                          ...
      ...
      1, bindent, pbs.fidma  );
   printmline( '      ''cJxv'',''Lxy'',''Lgxy'',''LgHxy'',''LIxy'',''LIgxy'',''LIgHxy'',''LHxyv'',''LIHxyv''}',   ...
      1, bindent, pbs.fidma  );
   printmline( ' ',                                                                           0,      bindent, pbs.fidma );
   ...
   printmline( '[varargout{1:max(1,nargout)}] = s2xlib(action,pbm,varargin{:});',             3,      bindent, pbs.fidma );
...
case 'python'
   %  Nothing to do here: everything is managed within s2xlib.py.
case 'julia'
   printjline( ' ',                                                                           0,      bindent, pbs.fidjl );
   printjline( '#%%%%%%%%%%%%%%% THE MAIN ACTIONS %%%%%%%%%%%%%%%',                           1,      bindent, pbs.fidjl );
   printjline( ' ',                                                                           0,      bindent, pbs.fidjl );
   printjline( 'elseif action in  ["fx","fgx","fgHx","cx","cJx","cJHx","cIx","cIJx","cIJHx","cIJxv","fHxv","cJxv","Lxy","Lgxy","LgHxy","LIxy","LIgxy","LIgHxy","LHxyv","LIHxyv"]',                                                                   ...
      1, bindent, pbs.fidjl  ); 
   printjline( ' ',                                                                           0,      bindent, pbs.fidjl );
   printjline( 'pbm = args[1]',                                                               2,      bindent, pbs.fidjl );
   ...
   printjline( 'return s2x_eval(action,args...)',                                             3,      bindent, pbs.fidjl );
...
case 'matlab'
   ...
   printmline( 'disp(['' ERROR: unknown action '',action,'' requested from '',name,''.m''])', 2,      bindent, pbs.fidma );
...
case 'julia'
   ...
   printjline( 'println("ERROR: unknown action "*action*" requested from "*name*"%s.jl")',    2,      bindent, pbs.fidjl );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Transform a Fortran expression to one (hopefully) acceptable for S2X, depending on the output file language.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [ thename, isidx, explindex ] = s2xname( nametype, sifname, pbs )
...
%  Fortran-based statements), for which an unquoted naming convention is necessary to pass their values to s2xlib. The
%  s2x internal pbs structure is needed in input because knowledge of loop indeces is necessary to verify if sifname is
%  one of them. The flag isidx is returned true iff sifname is the name of an active loop' index, in which case its value
%  is well-defined in the output file and can be used in the s2xvalue function without recourse to the dictionary
...
function pvalue = s2xvalue( nametype, sifname, pbs )
...
[ thename, isidx ] = s2xname( nametype, sifname, pbs );
if ( isidx  )
   pvalue = thename;
else
