function EDENSCH(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling EDENSCH.")
    end
    ...
    if action == "setup"
        ...
        ig,ig_,_ = s2mpj_ii("A"*string(Int64(v_["N"])),ig_)
        arrset(gtype,ig,"<>")
        push!(irA,ig)
        push!(icA,ix_["X"*string(Int64(v_["N"]))])
        push!(valA,Float64(0.0))
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        pb.n   = length(ix_)
        ngrp   = length(ig_)
        ...
        for I = Int64(v_["1"]):Int64(v_["N-1"])
            pbm.gconst[ig_["A"*string(I)]] = Float64(2.0)
            pbm.gconst[ig_["C"*string(I)]] = Float64(-1.0)
        end
        pbm.gconst[ig_["A"*string(Int64(v_["N"]))]] = Float64(2.0)
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for I = Int64(v_["1"]):Int64(v_["N-1"])
            ...
            vname = "X"*string(I)
            iv,ix_,pb = s2mpj_nlx(vname,ix_,pb,1,nothing,nothing,Float64(8.0))
