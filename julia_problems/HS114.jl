function HS114(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling HS114.")
    end
    ...
    if action == "setup"
        ...
        push!(irA,ig)
        push!(icA,ix_["X9"])
        push!(valA,Float64(v_["INVB"]))
        ig,ig_,_ = s2mpj_ii("C4",ig_)
        arrset(gtype,ig,">=")
        arrset(pb.cnames,ig,"C4")
        push!(irA,ig)
        push!(icA,ix_["X10"])
        push!(valA,Float64(v_["INVA"]))
        push!(irA,ig)
        push!(icA,ix_["X7"])
        push!(valA,Float64(-3.0))
        ...
        push!(irA,ig)
        push!(icA,ix_["X8"])
        push!(valA,Float64(1.098))
        push!(irA,ig)
        push!(icA,ix_["X6"])
        push!(valA,Float64(0.325))
        push!(irA,ig)
        push!(icA,ix_["X7"])
        push!(valA,Float64(v_["-A"]))
