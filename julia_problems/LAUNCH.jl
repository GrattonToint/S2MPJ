function LAUNCH(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling LAUNCH.")
    end
    ...
    if action == "setup"
        ...
        push!(irA,ig)
        push!(icA,ix_["ET3"])
        push!(valA,Float64(0.001958))
        arrset(pbm.gscale,ig,Float64(1.0e+8))
        ig,ig_,_ = s2mpj_ii("INST",ig_)
        arrset(gtype,ig,"<>")
        push!(irA,ig)
        push!(icA,ix_["INW"])
        push!(valA,Float64(47.040096))
        arrset(pbm.gscale,ig,Float64(1.0e+8))
        ig,ig_,_ = s2mpj_ii("LAUN",ig_)
        arrset(gtype,ig,"<>")
        push!(irA,ig)
        push!(icA,ix_["PW1"])
        push!(valA,Float64(0.003))
        push!(irA,ig)
        push!(icA,ix_["PW2"])
        push!(valA,Float64(0.003))
        push!(irA,ig)
