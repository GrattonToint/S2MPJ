function SYNTHES3(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling SYNTHES3.")
    end
    ...
    if action == "setup"
        ...
        push!(irA,ig)
        push!(icA,ix_["X4"])
        push!(valA,Float64(80.0))
        push!(irA,ig)
        push!(icA,ix_["X5"])
        push!(valA,Float64(25.0))
        push!(irA,ig)
        push!(icA,ix_["X6"])
        push!(valA,Float64(35.0))
        push!(irA,ig)
        push!(icA,ix_["X7"])
        push!(valA,Float64(-40.0))
        push!(irA,ig)
        push!(icA,ix_["X8"])
        push!(valA,Float64(15.0))
        push!(irA,ig)
