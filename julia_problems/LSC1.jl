function LSC1(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling LSC1.")
    end
    ...
    if action == "setup"
        ...
        if haskey(ix_,"X")
            pb.x0[ix_["X"]] = Float64(105.0)
        else
            pb.y0[findfirst(x->x==ig_["X"],pbm.congrps)] = Float64(105.0)
        end
        if haskey(ix_,"Y")
            pb.x0[ix_["Y"]] = Float64(96.0)
        else
            pb.y0[findfirst(x->x==ig_["Y"],pbm.congrps)] = Float64(96.0)
        end
        if haskey(ix_,"R")
            pb.x0[ix_["R"]] = Float64(230.0)
        else
            pb.y0[findfirst(x->x==ig_["R"],pbm.congrps)] = Float64(230.0)
        end
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
