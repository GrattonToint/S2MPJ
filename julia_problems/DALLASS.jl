function DALLASS(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling DALLASS.")
    end
    ...
    if action == "setup"
        ...
        push!(irA,ig)
        push!(icA,ix_["X42"])
        push!(valA,Float64(-6.38400e+02))
        push!(irA,ig)
        push!(icA,ix_["X43"])
        push!(valA,Float64(-6.33000e+02))
        push!(irA,ig)
        push!(icA,ix_["X44"])
        push!(valA,Float64(-5.54500e+02))
        push!(irA,ig)
        push!(icA,ix_["X45"])
        push!(valA,Float64(-5.05000e+02))
        push!(irA,ig)
        push!(icA,ix_["X46"])
        push!(valA,Float64(-4.36900e+02))
