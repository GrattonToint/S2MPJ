function LUKSAN12LS(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling LUKSAN12LS.")
    end
    ...
    if action == "setup"
        ...
        for J = Int64(v_["1"]):Int64(v_["S"])
            ...
            v_["K+4"] = 4+v_["K"]
            v_["K+5"] = 5+v_["K"]
            pbm.gconst[ig_["E"*string(Int64(v_["K+1"]))]] = Float64(1.0)
            pbm.gconst[ig_["E"*string(Int64(v_["K+4"]))]] = Float64(10.0)
            pbm.gconst[ig_["E"*string(Int64(v_["K+5"]))]] = Float64(20.0)
            v_["K"] = 6+v_["K"]
        end
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
        ...
        for J = Int64(v_["1"]):Int64(v_["S"])
            ...
            posev = findfirst(x->x=="X0",elftv[ielftype[ie]])
            ...
            ie,ie_,_  = s2mpj_ii(ename,ie_)
            arrset(pbm.elftype,ie,"eE4")
            arrset(ielftype,ie,iet_["eE4"])
            ename = "E"*string(Int64(v_["K+3"]))
            ie,ie_,_  = s2mpj_ii(ename,ie_)
            ...
            posev = findfirst(x->x=="X3",elftv[ielftype[ie]])
            loaset(pbm.elvar,ie,posev,iv)
            ename = "F"*string(Int64(v_["K+4"]))
            ie,ie_,_  = s2mpj_ii(ename,ie_)
            arrset(pbm.elftype,ie,"eF5")
            arrset(ielftype,ie,iet_["eF5"])
            ename = "F"*string(Int64(v_["K+4"]))
            ie,ie_,_  = s2mpj_ii(ename,ie_)
            vname = "X"*string(Int64(v_["I+3"]))
            iv,ix_,pb = s2mpj_nlx(vname,ix_,pb,1,Float64(-Inf),Float64(Inf),nothing)
        ...
        for J = Int64(v_["1"]):Int64(v_["S"])
            ...
            posel = length(pbm.grelt[ig])+1
            loaset(pbm.grelt,ig,posel,ie_["E"*string(Int64(v_["K+4"]))])
            loaset(pbm.grelw,ig,posel,1.)
            posel = posel+1
            loaset(pbm.grelt,ig,posel,ie_["F"*string(Int64(v_["K+4"]))])
            loaset(pbm.grelw,ig,posel, 1.)
...
# **********************
    ...
    elseif action == "eE5"
        ...
        nargout = args[3]
        pbm     = args[4]
        f_   = EV_[2]*EV_[1]*EV_[1]
        if nargout>1
            dim = try length(IV_) catch; length(EV_) end
            g_  = zeros(Float64,dim)
            g_[1] = 2.0e0*EV_[2]*EV_[1]
            g_[2] = EV_[1]*EV_[1]
            if nargout>2
    ...
    elseif action == "eE6"
        ...
        if nargout>1
            dim = try length(IV_) catch; length(EV_) end
            g_  = zeros(Float64,dim)
            g_[1] = 4.0e0*(EV_[1]^3)*(EV_[2]^2)
            g_[2] = 2.0e0*(EV_[1]^4)*EV_[2]
            if nargout>2
                H_ = zeros(Float64,2,2)
