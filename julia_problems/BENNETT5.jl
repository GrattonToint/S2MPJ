function BENNETT5(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling BENNETT5.")
    end
    ...
    if action == "setup"
        ...
        if haskey(ix_,"B1")
            ...
            pb.y0[findfirst(x->x==ig_["B1"],pbm.congrps)] = Float64(-2000.0)
        end
        if haskey(ix_,"B2")
            pb.x0[ix_["B2"]] = Float64(50.0)
        else
            pb.y0[findfirst(x->x==ig_["B2"],pbm.congrps)] = Float64(50.0)
        end
        if haskey(ix_,"B3")
            pb.x0[ix_["B3"]] = Float64(0.8)
        else
            pb.y0[findfirst(x->x==ig_["B3"],pbm.congrps)] = Float64(0.8)
        end
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        ...
        for I = Int64(v_["1"]):Int64(v_["M"])
            ...
            arrset(pbm.elftype,ie,"eE15")
            arrset(ielftype,ie,iet_["eE15"])
            vname = "B1"
...
# **********************
    ...
    elseif action == "eE15"
        ...
        if nargout>1
            ...
            g_[1] = 1.0/V2PXP
            g_[2] = -EV_[1]/(EV_[3]*V2PXP1)
            g_[3] = EV_[1]*V2PXL/(V2PXP*EV_[3]^2)
            if nargout>2
