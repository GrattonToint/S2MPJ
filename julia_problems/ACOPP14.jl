...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if action == "setup"
        ...
        pbm.A[ig,iv] += Float64(4000.0)
        for I = Int64(v_["1"]):Int64(v_["NODES"])
            ig,ig_,_ = s2x_ii("RP"*string(I),ig_)
            arrset(gtype,ig,"==")
            arrset(pb.cnames,ig,"RP"*string(I))
        end
        for I = Int64(v_["1"]):Int64(v_["NODES"])
            ig,ig_,_ = s2x_ii("IP"*string(I),ig_)
            arrset(gtype,ig,"==")
        ...
        for I = Int64(v_["1"]):Int64(v_["LINES"])
            ig,ig_,_ = s2x_ii("FN"*string(I),ig_)
            arrset(gtype,ig,"<=")
            arrset(pb.cnames,ig,"FN"*string(I))
        end
        for I = Int64(v_["1"]):Int64(v_["LINES"])
            ig,ig_,_ = s2x_ii("TN"*string(I),ig_)
            arrset(gtype,ig,"<=")
