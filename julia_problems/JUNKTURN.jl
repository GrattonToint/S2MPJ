...
# IE N                   100000         $-PARAMETER n = 1000010, m = 700000
        ...
        for I = Int64(v_["1"]):Int64(v_["7"])
            for T = Int64(v_["0"]):Int64(v_["N"])
                iv,ix_,_ = s2x_ii("X"*string(I)*","*string(T),ix_)
        ...
        for T = Int64(v_["1"]):Int64(v_["N"])
            v_["T-1"] = -1+T
            for I = Int64(v_["1"]):Int64(v_["7"])
                ig,ig_,_ = s2x_ii("C"*string(I)*","*string(T),ig_)
                arrset(gtype,ig,"==")
