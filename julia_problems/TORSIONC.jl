function TORSIONC(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling TORSIONC.")
    end
...
# IE Q                   61             $-PARAMETER n= 14884
        if nargin<2
            v_["C"] = Float64(10.0);  #  SIF file default value
        else
            v_["C"] = Float64(args[2]);
        end
        v_["Q+1"] = 1+v_["Q"]
        ...
        for J = Int64(v_["1"]):Int64(v_["P"])
            ...
            pb.x0[ix_["X"*string(Int64(v_["P"]))*","*string(J)]] = Float64(0.0)
        end
        for I = Int64(v_["2"]):Int64(v_["P-1"])
            pb.x0[ix_["X"*string(I)*","*string(Int64(v_["P"]))]] = Float64(0.0)
        ...
        for I = Int64(v_["2"]):Int64(v_["Q"])
            for J = Int64(v_["2"]):Int64(I)
                ...
                v_["RJ-1"] = Float64(v_["J-1"])
                v_["UPPL"] = v_["RJ-1"]*v_["H"]
                pb.x0[ix_["X"*string(I)*","*string(J)]] = Float64(v_["UPPL"])
            end
            v_["MI"] = -1*I
            v_["P-I"] = v_["P"]+v_["MI"]
            v_["I-1"] = -1+I
            ...
            v_["P-I+1"] = 1+v_["P-I"]
            for J = Int64(I):Int64(v_["P-I+1"])
                pb.x0[ix_["X"*string(I)*","*string(J)]] = Float64(v_["UPPM"])
            end
            for J = Int64(v_["P-I+1"]):Int64(v_["P-1"])
                v_["MJ"] = -1*J
                v_["P-J"] = v_["P"]+v_["MJ"]
                v_["RP-J"] = Float64(v_["P-J"])
                v_["UPPR"] = v_["RP-J"]*v_["H"]
                pb.x0[ix_["X"*string(I)*","*string(J)]] = Float64(v_["UPPR"])
            end
        end
        for I = Int64(v_["Q+1"]):Int64(v_["P-1"])
            v_["MI"] = -1*I
