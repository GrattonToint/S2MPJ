function SIPOW4(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling SIPOW4.")
    end
    ...
    if action == "setup"
        ...
        for J = Int64(v_["1"]):Int64(v_["M/2"])
            ...
            push!(irA,ig)
            push!(icA,ix_["X2"])
            push!(valA,Float64(v_["XI"*string(J)]))
            push!(irA,ig)
            push!(icA,ix_["X3"])
            push!(valA,Float64(v_["ETA"*string(J)]))
        end
        for J = Int64(v_["1"]):Int64(v_["M/2"])
            v_["J+"] = v_["M/2"]+J
