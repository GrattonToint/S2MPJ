function CAMSHAPE(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling CAMSHAPE.")
    end
    ...
    if action == "setup"
        ...
        push!(irA,ig)
        push!(icA,ix_["R"*string(Int64(v_["1"]))])
        push!(valA,Float64(v_["-RMIN"]))
        push!(irA,ig)
        push!(icA,ix_["R"*string(Int64(v_["2"]))])
        push!(valA,Float64(v_["RMIN2CD"]))
        ...
        push!(irA,ig)
        push!(icA,ix_["R"*string(Int64(v_["1"]))])
        push!(valA,Float64(v_["R"]))
        ig,ig_,_ = s2mpj_ii("E3",ig_)
        arrset(gtype,ig,"<=")
        arrset(pb.cnames,ig,"E3")
        push!(irA,ig)
        push!(icA,ix_["R"*string(Int64(v_["N"]))])
        push!(valA,Float64(v_["-RMAX"]))
        push!(irA,ig)
        push!(icA,ix_["R"*string(Int64(v_["N-1"]))])
        push!(valA,Float64(v_["RMAX2CD"]))
        ig,ig_,_ = s2mpj_ii("E4",ig_)
        arrset(gtype,ig,"<=")
        arrset(pb.cnames,ig,"E4")
        push!(irA,ig)
        push!(icA,ix_["R"*string(Int64(v_["N"]))])
        push!(valA,Float64(v_["-2RMAX"]))
