function TRAINH(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling TRAINH.")
    end
...
# IE N                   5001           $-PARAMETER n=20008, m=10002
        ...
        for I = Int64(v_["0"]):Int64(v_["N-1"])
            ...
            push!(valA,Float64(-1.0))
            push!(irA,ig)
            push!(icA,ix_["V"*string(Int64(v_["I+1"]))])
            push!(valA,Float64(v_["-H/2"]))
            push!(irA,ig)
            push!(icA,ix_["V"*string(I)])
            push!(valA,Float64(v_["-H/2"]))
            ...
            push!(irA,ig)
            push!(icA,ix_["V"*string(Int64(v_["I+1"]))])
            push!(valA,Float64(v_["1+BH/2"]))
            push!(irA,ig)
            push!(icA,ix_["V"*string(I)])
            push!(valA,Float64(v_["BH/2-1"]))
            push!(irA,ig)
            push!(icA,ix_["UA"*string(Int64(v_["I+1"]))])
            push!(valA,Float64(v_["-H/2"]))
            push!(irA,ig)
            push!(icA,ix_["UA"*string(I)])
            push!(valA,Float64(v_["-H/2"]))
            push!(irA,ig)
            push!(icA,ix_["UB"*string(Int64(v_["I+1"]))])
            push!(valA,Float64(v_["-H/2"]))
            push!(irA,ig)
            push!(icA,ix_["UB"*string(I)])
            push!(valA,Float64(v_["-H/2"]))
        end
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        pb.n   = length(ix_)
