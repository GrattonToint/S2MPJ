function AVION2(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling AVION2.")
    end
    ...
    if action == "setup"
        ...
        push!(irA,ig)
        push!(icA,ix_["QF"])
        push!(valA,Float64(v_["1/790"]))
        ig,ig_,_ = s2mpj_ii("E17",ig_)
        arrset(gtype,ig,"==")
        arrset(pb.cnames,ig,"E17")
        push!(irA,ig)
        push!(icA,ix_["IMPTRAIN"])
        push!(valA,Float64(1.0))
        push!(irA,ig)
        push!(icA,ix_["MV"])
        push!(valA,Float64(-0.137))
        ...
        push!(irA,ig)
        push!(icA,ix_["IMPNMOT"])
        push!(valA,Float64(1.0))
        push!(irA,ig)
        push!(icA,ix_["NM"])
        push!(valA,Float64(-35.0))
        ...
        push!(irA,ig)
        push!(icA,ix_["NP"])
        push!(valA,Float64(95.0))
        push!(irA,ig)
        push!(icA,ix_["NG"])
        push!(valA,Float64(70.0))
        push!(irA,ig)
        push!(icA,ix_["NM"])
        push!(valA,Float64(660.0))
        push!(irA,ig)
        push!(icA,ix_["QI"])
