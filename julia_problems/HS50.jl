function HS50(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling HS50.")
    end
    ...
    if action == "setup"
        ...
        if haskey(ix_,"X1")
            pb.x0[ix_["X1"]] = Float64(35.0)
        else
            pb.y0[findfirst(x->x==ig_["X1"],pbm.congrps)] = Float64(35.0)
        end
        if haskey(ix_,"X2")
            pb.x0[ix_["X2"]] = Float64(-31.0)
        else
            pb.y0[findfirst(x->x==ig_["X2"],pbm.congrps)] = Float64(-31.0)
        end
        if haskey(ix_,"X3")
            pb.x0[ix_["X3"]] = Float64(11.0)
        else
            pb.y0[findfirst(x->x==ig_["X3"],pbm.congrps)] = Float64(11.0)
        end
        if haskey(ix_,"X4")
