function JANNSON3(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling JANNSON3.")
    end
    ...
    if action == "setup"
        ...
        ig_ = Dict{String,Int}();
        if nargin<1
            v_["N"] = Int64(3);  #  SIF file default value
        else
            v_["N"] = Int64(args[1]);
        end
        v_["0"] = 0
        ...
        arrset(gtype,ig,"<=")
        arrset(pb.cnames,ig,"Q")
        ig,ig_,_ = s2mpj_ii("P",ig_)
        arrset(gtype,ig,"<=")
        arrset(pb.cnames,ig,"P")
        #%%%%%%%%%%%%%% GLOBAL DIMENSIONS %%%%%%%%%%%%%%%%%
        pb.n   = length(ix_)
        ngrp   = length(ig_)
        ...
        pbm.gconst[ig_["G0"]] = Float64(1.0)
        for I = Int64(v_["1"]):Int64(v_["2N"])
            pbm.gconst[ig_["G"*string(I)]] = Float64(1.0)
        end
        pbm.gconst[ig_["L"]] = Float64(1.0)
        pbm.gconst[ig_["Q"]] = Float64(0.75)
        pbm.gconst[ig_["P"]] = Float64(v_["1/5N"])
        #%%%%%%%%%%%%%%%%%%%  BOUNDS %%%%%%%%%%%%%%%%%%%%%
