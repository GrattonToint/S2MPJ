function CYCLOOCFLS(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling CYCLOOCFLS.")
    end
...
# IE P                   100000         $-PARAMETER
        ...
        pb.x0[ix_["Y2"]] = Float64(v_["START"])
        pb.x0[ix_["Z3"]] = Float64(v_["START"])
        for I = Int64(v_["3"]):Int64(v_["P"])
            v_["RI"] = Float64(I)
            v_["START"] = v_["RI"]/v_["RP"]
        ...
        v_["I"] = 1
        ig = ig_["A"*string(Int64(v_["I"]))]
        posel = length(pbm.grelt[ig])+1
        loaset(pbm.grelt,ig,posel,ie_["AY"*string(Int64(v_["I"]))])
        loaset(pbm.grelw,ig,posel,1.)
        posel = length(pbm.grelt[ig])+1
        loaset(pbm.grelt,ig,posel,ie_["AZ"*string(Int64(v_["I"]))])
        loaset(pbm.grelw,ig,posel,1.)
        ig = ig_["B"*string(Int64(v_["I"]))]
        posel = length(pbm.grelt[ig])+1
        loaset(pbm.grelt,ig,posel,ie_["BX"*string(Int64(v_["I"]))])
        loaset(pbm.grelw,ig,posel,1.)
        posel = length(pbm.grelt[ig])+1
        loaset(pbm.grelt,ig,posel,ie_["BY"*string(Int64(v_["I"]))])
        loaset(pbm.grelw,ig,posel,1.)
        posel = length(pbm.grelt[ig])+1
        loaset(pbm.grelt,ig,posel,ie_["BZ"*string(Int64(v_["I"]))])
        loaset(pbm.grelw,ig,posel,1.)
