function HS46(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling HS46.")
    end
    ...
    if action == "setup"
        ...
        if haskey(ix_,"X1")
            pb.x0[ix_["X1"]] = Float64(v_["X01"])
        else
            pb.y0[findfirst(x->x==ig_["X1"],pbm.congrps)] = Float64(v_["X01"])
        end
        if haskey(ix_,"X2")
            pb.x0[ix_["X2"]] = Float64(1.75)
        else
            pb.y0[findfirst(x->x==ig_["X2"],pbm.congrps)] = Float64(1.75)
        end
        if haskey(ix_,"X3")
            pb.x0[ix_["X3"]] = Float64(0.5)
        else
            pb.y0[findfirst(x->x==ig_["X3"],pbm.congrps)] = Float64(0.5)
        end
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
        ...
        vname = "X1"
        iv,ix_,pb  = (
              s2mpj_nlx(vname,ix_,pb,1,Float64(-Inf),Float64(Inf),Float64(2.0)))
        ...
        ename = "E2"
        ie,ie_,_  = s2mpj_ii(ename,ie_)
        arrset(pbm.elftype,ie,"eSPW")
        arrset(ielftype,ie,iet_["eSPW"])
        vname = "X3"
        iv,ix_,pb  = (
              s2mpj_nlx(vname,ix_,pb,1,Float64(-Inf),Float64(Inf),Float64(2.0)))
        posev = findfirst(x->x=="V1",elftv[ielftype[ie]])
        ...
        ename = "E5"
        ie,ie_,_  = s2mpj_ii(ename,ie_)
        arrset(pbm.elftype,ie,"eV2W")
        arrset(ielftype,ie,iet_["eV2W"])
        vname = "X1"
        iv,ix_,pb  = (
              s2mpj_nlx(vname,ix_,pb,1,Float64(-Inf),Float64(Inf),Float64(2.0)))
        posev = findfirst(x->x=="V1",elftv[ielftype[ie]])
        ...
        ename = "E7"
        ie,ie_,_  = s2mpj_ii(ename,ie_)
        arrset(pbm.elftype,ie,"eV4W2")
        arrset(ielftype,ie,iet_["eV4W2"])
        vname = "X3"
        iv,ix_,pb  = (
              s2mpj_nlx(vname,ix_,pb,1,Float64(-Inf),Float64(Inf),Float64(2.0)))
        posev = findfirst(x->x=="V1",elftv[ielftype[ie]])
...
# **********************
    ...
    elseif action == "eSPW"
        ...
        if nargout>1
            dim = try length(IV_) catch; length(EV_) end
            g_  = zeros(Float64,dim)
            g_[1] = pbm.elpar[iel_][1]*V1M1^(IPOW-1)
            if nargout>2
                H_ = zeros(Float64,1,1)
