function HAHN1(action::String,args::Union{Any}...)
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 
# 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( !isdefined(@__MODULE__, :s2mpj_ii) )
        error( "Please include(\"s2mpjlib.jl\") using \"s2mpjlib.jl\" from the S2MPJ distribution before calling HAHN1.")
    end
    ...
    if action == "setup"
        ...
        if haskey(ix_,"B1")
            pb.x0[ix_["B1"]] = Float64(10.0)
        else
            pb.y0[findfirst(x->x==ig_["B1"],pbm.congrps)] = Float64(10.0)
        end
        if haskey(ix_,"B2")
        ...
        if haskey(ix_,"B3")
            pb.x0[ix_["B3"]] = Float64(0.05)
        else
            pb.y0[findfirst(x->x==ig_["B3"],pbm.congrps)] = Float64(0.05)
        end
        if haskey(ix_,"B4")
            pb.x0[ix_["B4"]] = Float64(-0.00001)
        else
            pb.y0[findfirst(x->x==ig_["B4"],pbm.congrps)] = Float64(-0.00001)
        end
        if haskey(ix_,"B5")
            pb.x0[ix_["B5"]] = Float64(-0.05)
        else
            pb.y0[findfirst(x->x==ig_["B5"],pbm.congrps)] = Float64(-0.05)
        end
        if haskey(ix_,"B6")
            pb.x0[ix_["B6"]] = Float64(0.001)
        else
            pb.y0[findfirst(x->x==ig_["B6"],pbm.congrps)] = Float64(0.001)
        end
        if haskey(ix_,"B7")
            pb.x0[ix_["B7"]] = Float64(-0.000001)
        else
            pb.y0[findfirst(x->x==ig_["B7"],pbm.congrps)] = Float64(-0.000001)
        end
        #%%%%%%%%%%%%%%%%%%%% ELFTYPE %%%%%%%%%%%%%%%%%%%%%
